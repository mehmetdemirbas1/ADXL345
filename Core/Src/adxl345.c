...
uint8_t ADXL345_ScanDeviceAddr(I2C_HandleTypeDef *i2cHandle)
{
	for(uint8_t devAddr=0; devAddr<255;devAddr++){
		if(HAL_I2C_IsDeviceReady(i2cHandle, devAddr, 1, TIMEOUT)== HAL_OK)
			return devAddr;
	}
	return -1;
}
...
{
	if(HAL_I2C_Mem_Read(i2cHandle, ADXL345_DEVICE_ADDR, registerAddr, 1, pdata, sizeOfData, TIMEOUT)==HAL_OK){
		return READ_OK;
	}
	return READ_FAIL;
}
...
ADXL345_WriteStatus_t ADXL345_WriteRegister(I2C_HandleTypeDef *i2cHandle, uint16_t registerAddr, uint16_t Value)
{
	...
	if(HAL_I2C_Master_Transmit(i2cHandle, ADXL345_DEVICE_ADDR, data, sizeof(data), TIMEOUT)== HAL_OK)
		return WRITE_OK;
...
{
	uint8_t deviceId = 0;
	ADXL345_ReadRegister(i2cHandle, DEVID, sizeof(uint8_t),&deviceId);
	if(0xE5 != deviceId)
		return INIT_FAIL;
	...
	ADXL345_WriteRegister(i2cHandle,POWER_CTL, temp);
	...
	ADXL345_WriteRegister(i2cHandle,DATA_FORMAT, temp);
	...
	ADXL345_WriteRegister(i2cHandle,BW_RATE, temp);
...
int16_t ADXL345_getAxisValue(I2C_HandleTypeDef *i2cHandle, uint8_t axisValue)
{
	...
	ADXL345_ReadRegister(i2cHandle,axisValue, 2, data);
...
{
	double outputData = 0;
	outputData =(double) ADXL345_getAxisValue(i2cHandle,axisValue);
